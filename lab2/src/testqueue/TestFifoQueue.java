...
public class TestFifoQueue {
	...
	public final void testEmptyIterator() {
		assertTrue("Should be empty", !myIntQueue.iterator().hasNext());
	}
	
	@Test
	public final void testMakeEmptyIterator() {
		myIntQueue.offer(1);
		Iterator<Integer> itr = myIntQueue.iterator();
		assertEquals("Should return 1", (Integer)1, itr.next());
		assertTrue("Should be empty", !itr.hasNext());
	}
	
	@Test
	public final void testPullElements() {
		myIntQueue.offer(1);
		myIntQueue.offer(2);
		myIntQueue.offer(3);
		Iterator<Integer> itr = myIntQueue.iterator();
		assertEquals("Should return 1", (Integer)1, itr.next());
		assertEquals("Should return 2", (Integer)2, itr.next());
		assertEquals("Should return 3", (Integer)3, itr.next());
		assertTrue("Should be empty", !itr.hasNext());
	}
